...
import common._
import loadbalancer.{PartitionedLoadBalancerFactory, PartitionedLoadBalancer, DefaultClusteredLoadBalancerFactory, PartitionedLoadBalancerFactoryComponent}
import server.{MessageExecutorComponent, NetworkServer}
import netty.{NettyClusteredNetworkClient}
...
object ClusteredNetworkClient {
  def apply[PartitionedId](config: NetworkClientConfig, loadBalancerFactory: PartitionedLoadBalancerFactory[PartitionedId]):
  ClusteredNetworkClient[PartitionedId] = {
    val nc = new NettyClusteredNetworkClient(config, loadBalancerFactory)
    nc.start
    nc
  }
