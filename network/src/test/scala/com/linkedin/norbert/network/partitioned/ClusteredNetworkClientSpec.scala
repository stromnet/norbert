...
class ClusteredNetworkClientSpec extends BaseNetworkClientSpecification {
  val networkClient = new ClusteredNetworkClient[Int] with ClusterClientComponent with ClusterIoClientComponent
    ...
    val clusterClient = ClusteredNetworkClientSpec.this.clusterClient
  }

//  networkClient.messageRegistry.contains(any[Message]) returns true

  "ClusteredNetworkClient" should {
...
//      clusterIoClient.sendRequest(node, message, null) wasnt called
    ...
    "retryCallback should propagate server exception to underlying when" in {
      ...
      "sendMessage: MAX_RETRY reached" in {
        val nc2 = new ClusteredNetworkClient[Int] with ClusterClientComponent with ClusterIoClientComponent with
          PartitionedLoadBalancerFactoryComponent[Int] {
          val lb = new PartitionedLoadBalancer[Int] {
            var iter = ClusteredNetworkClientSpec.this.nodes.iterator
            def nextNode(id: Int, c: Option[Long] = None, pc : Option[Long] = None) = {
              if (!iter.hasNext ) iter = ClusteredNetworkClientSpec.this.nodes.iterator
              Some(iter.next)
            }
          ...
          val clusterIoClient = new ClusterIoClient {
            ...
            def nodesChanged(nodes: Set[Node]) = {ClusteredNetworkClientSpec.this.endpoints}
            def shutdown {}
          }
          val clusterClient = ClusteredNetworkClientSpec.this.clusterClient
        }
    ...
    "sendMessage should automatically handle partial failures" in {
      val MAX_RETRY = 3
      val nc2 = new ClusteredNetworkClient[Int] with ClusterClientComponent with ClusterIoClientComponent with PartitionedLoadBalancerFactoryComponent[Int] {
        val lb = new PartitionedLoadBalancer[Int] {
          val nodeIS = ClusteredNetworkClientSpec.this.nodes.toIndexedSeq
          var idx = 0
          def nextNode(id: Int, c: Option[Long] = None, pc: Option[Long] = None) = {
    ...
    "calculateNodesFromIds should properly exclude failing node" in {
      val nc2 = new ClusteredNetworkClient[Int] with ClusterClientComponent with ClusterIoClientComponent with PartitionedLoadBalancerFactoryComponent[Int] {
        val lb = new PartitionedLoadBalancer[Int] {
          val iter = ClusteredNetworkClientSpec.this.nodes.iterator
